...
import { sanitizeSchedule } from "@/lib/coaches";

export const dynamic = "force-dynamic";

// PATCH /api/admin/coaches
// Body: { slug, bio, role, booking_schedule }
// Updates a coach's editable booking profile (availability + public bio/role).
// These live in dedicated columns, separate from the CRM's own description/role.
// Admin only.
export async function PATCH(req: NextRequest) {
  ...
  const rows = (await sql`
    ...
    SET booking_bio = ${bio},
        ...
        updated_at = now()
    WHERE slug = ${slug}
    RETURNING slug, booking_bio, booking_role, booking_schedule
  `) as unknown as Array<{
    slug: string;
